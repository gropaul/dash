...
def test_json_simple(http_duck: Client):
    res = http_duck.execute_query("SELECT 1 AS number, 'text' AS text", response_format=ResponseFormat.JSON)
    assert res == {"data": [{"number": 1, "text": "text"}]}
